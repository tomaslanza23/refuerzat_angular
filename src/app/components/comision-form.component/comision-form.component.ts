...
export class ComisionFormComponent implements OnInit {
  ...
  private cargarProgramas(): void {
    this.programaService.getAllProgramas().subscribe({
      next: (programas) => { this.programas = programas.filter(p => p.activo); },
      error: () => { this.error = 'Error al cargar los programas'; }
    });
  }
  ...
  private cargarComision(id: number): void {
    ...
    this.comisionService.getComision(id).subscribe({
      ...
      error: () => { this.error = 'Error al cargar la comisión'; this.cargando = false; }
    });
  }

  cancelar(): void { this.router.navigate(['/admin/comisiones']); }
  ...
  guardar(): void {
    ...
    if (this.esEdicion && nuevoCupo < this.cuposOcupadosOriginales) {
      this.error = `El cupo no puede ser menor a los cupos ocupados (${this.cuposOcupadosOriginales})`;
      return;
    }
  ...
  private crear(): void {
    ...
    this.comisionService.crearComision({
      ...
      error: err => {
        const field = err.error?.field ?? err.error;
        if (field === 'codigoComision') this.error = 'Ya existe una comisión con ese código';
        else if (field === 'programaId') this.error = 'El programa no existe o está inactivo';
        else this.error = 'Error al crear la comisión';
      }
    });
  }
  ...
  private actualizar(): void {
    ...
    this.comisionService.actualizarComision(this.idComision, {
      ...
      error: err => {
        const field = err.error?.field ?? err.error;
        if (field === 'codigoComision') this.error = 'Ya existe una comisión con ese código';
        else if (field === 'cupoTotal') this.error = `El cupo no puede ser menor a los cupos ocupados (${this.cuposOcupadosOriginales})`;
        else if (field === 'activa') this.error = 'No se puede desactivar una comisión con inscriptos';
        else this.error = 'Error al actualizar la comisión';
      }
    });
  }

  getCuposDisponibles(): number {
    return Math.max(0, (this.form.get('cupoTotal')?.value || 0) - this.cuposOcupadosOriginales);
  }
}